...
for (let p of targets) {
    const fullPath = path.join('c:/Users/aguir/Documents/Code/ekobusiness/frontend', p)
    ...
    let configJs = `export const columns = ${configCols ? configCols.replace(/^columns:\s*/, '') : '[]'}\n`
    if (configOpts)
        configJs += `\nexport const tableRowOptions = ${configOpts.replace(/^(tableRowOptions|options):\s*/, '')}\n`
    else configJs += `\nexport const tableRowOptions = []\n`
    fs.writeFileSync(configPath, configJs)

    if (configCols) {
        cnt = cnt.replace(configCols + ',\n\n', '')
        cnt = cnt.replace(configCols + ',\n', '')
        cnt = cnt.replace(configCols + ',', '')
        cnt = cnt.replace(configCols, '')
    }
    if (configOpts) {
        cnt = cnt.replace(configOpts + ',\n\n', '')
        cnt = cnt.replace(configOpts + ',\n', '')
        cnt = cnt.replace(configOpts + ',', '')
        cnt = cnt.replace(configOpts, '')
    }
    ...
    if (headMatch) {
        ...
        let beforeStrong = insideHead.split(/<strong>/)[0]
        ...
        <div class="head-right">
            <JdPaginacion :view="vista" @reload="${reloadMethod}" />
        ...
        cnt = cnt.replace(headMatch[0], newHead)
    }

    let headContainerMatch = cnt.match(
        /<div class="vista vista-fill">[\s\n]+<div class="head-left">/,
    )
    if (headContainerMatch) {
        cnt = cnt.replace(
            headContainerMatch[0],
            '<div class="vista vista-fill">\n        <div class="head">\n        <div class="head-left">',
        )

        let jdTableMatch = cnt.match(/<\/div>\s+<JdTable/)
        if (jdTableMatch) {
            cnt = cnt.replace(jdTableMatch[0], '</div>\n        </div>\n\n        <JdTable')
        }
    }

    // append ref and options
    let tMatch = cnt.match(/<JdTable([^>]+)>/)
    if (tMatch) {
        let props = tMatch[1]
        if (!props.includes('ref="jdtable"')) props += '\n            ref="jdtable"'
        if (!props.includes(':rowOptions')) props += '\n            :rowOptions="tableRowOptions"'
        if (!props.includes('@rowOptionSelected'))
