                ...
                <tbody>
                    <tr
                        ...
                        :class="{
                            ...
                            'row-reorderable': rowReorderable,
                        }"
                        @click="selectRow(a)"
                    ...
                    >
                        <td v-if="rowReorderable" class="td-reorder">
...
export default {
    ...
    methods: {
        ...
        handleDragStart(index, event) {
            ...
            event.currentTarget.classList.add('row-dragging')
        },
        handleDragOver(index, event) {
            if (this.draggedRowIndex === null) return
            if (this.draggedRowIndex === index) return
            ...
            if (event.clientY < midpoint) {
                ...
                tr.classList.remove('row-drag-over-bottom')
            } else {
                tr.classList.add('row-drag-over-bottom')
                tr.classList.remove('row-drag-over-top')
            }
        },
        ...
        handleDrop(index, event) {
            ...
            // eslint-disable-next-line vue/no-mutating-props
            const draggedItem = this.datos.splice(this.draggedRowIndex, 1)[0]
            // eslint-disable-next-line vue/no-mutating-props
            this.datos.splice(index, 0, draggedItem)

            // Update the reorder prop
            this.datos.forEach((item, idx) => {
                item[this.rowReorderProp] = idx + 1
            })

            this.draggedRowIndex = null
            this.$emit('onReorder', this.datos)
        },
        handleDragEnd(event) {
            event.currentTarget.classList.remove('row-dragging')
            this.draggedRowIndex = null

            const overs = this.$el.querySelectorAll('.row-drag-over-top, .row-drag-over-bottom')
            overs.forEach((el) => {
                el.classList.remove('row-drag-over-top')
                el.classList.remove('row-drag-over-bottom')
            })
        },
    },
}
