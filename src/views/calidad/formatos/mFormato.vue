    ...
    <JdModal modal="mFormato" :buttons="buttons" @button-click="(action) => this[action]()">
        ...
        <div class="container-datos">
            <template v-if="modal.transaccion_item1">
                <JdInput
                    label="Fecha"
                    v-model="modal.transaccion_item1.transaccion1.fecha"
                    :disabled="true"
                />
                <JdInput
                    label="Guía"
                    v-model="modal.transaccion_item1.transaccion1.guia"
                    :disabled="true"
                />
                <JdInput
                    :label="
                        modal.transaccion_item1.transaccion1.tipo == 1 ? 'Proveedor' : 'Cliente'
                    "
                    v-model="modal.transaccion_item1.transaccion1.socio1.nombres"
                    :disabled="true"
                />
                <JdInput
                    label="Artículo"
                    v-model="modal.transaccion_item1.articulo1.nombre"
                    :disabled="true"
                />
                <JdInput
                    label="Cantidad"
                    v-model="modal.transaccion_item1.cantidad"
                    :disabled="true"
                />
                ...
                <JdInput
                    ...
                    type="date"
                    v-model="modal.transaccion_item1.fv"
                    :disabled="true"
                />
            </template>
            ...
            <template v-if="modal.produccion_orden1">
                ...
                <JdInput
                    label="Máquina"
                    v-model="modal.produccion_orden1.maquina1.nombre"
                    :disabled="true"
                    v-if="modal.produccion_orden1.maquina1"
                />
                <JdInput
                    label="Artículo"
                    v-model="modal.produccion_orden1.articulo1.nombre"
                    :disabled="true"
                />
            </template>
            ...
            <template v-if="modal.transaccion1">
                ...
                <JdInput
                    ...
                    v-model="modal.transaccion1.guia"
                    :disabled="true"
                />
                <JdInput
                    label="Destino"
                    v-model="modal.transaccion1.socio1.nombres"
                    :disabled="true"
                />
            </template>
            ...
            <template v-if="modal.cuarentena_producto1">
                <JdInput
                    label="Fecha"
                    v-model="modal.cuarentena_producto1.produccion_orden1.fecha"
                    :disabled="true"
                />
                <JdInput
                    label="Producto"
                    v-model="modal.cuarentena_producto1.produccion_orden1.articulo1.nombre"
                    :disabled="true"
                />
                ...
                <JdInput
                    label="Cantidad"
                    v-model="modal.cuarentena_producto1.cantidad"
                    :disabled="true"
                />
            </template>
...
export default {
    ...
    methods: {
        ...
        shapeDatos() {
            ...
            const transaccion_item = this.modal.send.values.find(
                (a) => a.relacion == 'transaccion_items',
            )
            if (transaccion_item) transaccion_item.value = this.modal.transaccion_item

            const produccion_orden = this.modal.send.values.find(
            ...
            const cuarentena_producto = this.modal.send.values.find(
                (a) => a.relacion == 'cuarentena_productos',
            )
        ...
        async grabar() {
            ...
            const vista =
                this.modal.formato.tipo == 'BPM'
                    ? 'vFormatosBpm'
                    : this.modal.formato.tipo == 'PHS'
                      ? 'vFormatosPhs'
                      : 'vFormatosHaccp'
