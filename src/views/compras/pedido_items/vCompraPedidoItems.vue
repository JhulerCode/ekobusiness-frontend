...
export default {
    ...
    methods: {
        ...
        initFiltros() {
            if (!this.config.tableColumns[0].val) {
                this.config.tableColumns[0].op = 'Está dentro de'
                this.config.tableColumns[0].val = dayjs().startOf('month').format('YYYY-MM-DD')
                this.config.tableColumns[0].val1 = dayjs().format('YYYY-MM-DD')
            }
        },
        setQuery() {
            this.vista.qry = {
            ...
            this.useAuth.updateQuery(this.config.tableColumns, this.vista.qry)
        },
    },
}
</script>
