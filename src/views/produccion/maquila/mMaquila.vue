<template>
    <JdModal modal="mMaquila" :buttons="buttons" @button-click="(action) => this[action]()">
...
export default {
    ...
    created() {
        this.modal = this.useModals.mMaquila
    ...
    methods: {
        showButtons() {
            if (this.useModals.mMaquila.mode == 1) {
                this.buttons[0].show = true
            } else if (this.useModals.mMaquila.mode == 2) {
        ...
        async crear() {
            ...
            this.useModals.show.mMaquila = false
        },
        async modificar() {
            if (this.checkDatos()) return
